...
def setup_training(main_net, meta_net, exp_id=None):
    ...
    # meta net optimizer
    optimizer = SAM(main_params, torch.optim.SGD, rho = 2.0, adaptive= 'True', lr=args.main_lr, momentum=args.momentum, weight_decay=args.wdecay)
    scheduler = DummyScheduler(optimizer)

    # main net optimizer
    main_params = main_net.parameters() 
    if args.optimizer == 'sam':
        main_opt = SAM(main_params, torch.optim.SGD, rho = 2.0, adaptive= 'True', lr=args.main_lr, momentum=args.momentum, weight_decay=args.wdecay)
